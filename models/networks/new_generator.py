...
import networks.normalization as norms
    ...
    def __init__(self, opt):
        ...
        if not self.opt.no_3dnoise:
            ...
            self.seg_pyrmid.append(
                nn.Sequential(
                    nn.Conv2d(
                        self.opt.semantic_nc + self.opt.z_dim,
                        32,
                        3,
                        stride=1,
                        padding=1,
    ...
    def execute(self, input, z=None):
        ...
        for i in range(self.opt.num_res_blocks):
            seg_f = jt.concat(
                [
                    nn.interpolate(
                        pyrmid[-1], size=(x.shape[2], x.shape[3]), mode="bilinear"
                    ),
                    pyrmid[self.opt.num_res_blocks - i],
                ],
                1,
            )
